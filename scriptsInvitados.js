...
async function generarListaInvitados(filtro = "") {
  ...
  const querySnapshot = await db
    .collection("invitadosbodaAdeline2025")
  ...
  invitadosFiltrados.forEach((invitado) => {
    ...
    acompanantes.textContent = `Acompañantes: ${invitado.numeroInvitados}`;
...
window.agregarInvitado = async function () {
  ...
  if (nombreInvitado && numeroInvitados) {
    ...
    await db.collection("invitadosbodaAdeline2025").add({
...
window.eliminarInvitado = async function (id) {
  ...
  if (confirmacion) {
    mostrarCargando(true);
    await db.collection("invitadosbodaAdeline2025").doc(id).delete();
